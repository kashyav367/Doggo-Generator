...
fetch(BREEDS_URL)
    .then(res => {   // res is a parameter that is passing in the method
        return res.json()
    })
    .then(data => {
        const breedsObject = data.message; //Turn the message into the object
    ...
    select.addEventListener('change', event => {
        ...
        getDoggoImg(url);
        doggoInfo.assignMF() 
        doggoInfo.assignAge() // call a method
